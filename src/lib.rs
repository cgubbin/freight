...
pub fn build() -> Result<()> {
    ...
    // TODO: Recover from a configuration file
    ...
    match (lib_rs.exists(), main_rs.exists()) {
        (true, true) => {
            ...
            bin_compile(&mut logger, &manifest, &main_rs, &target_debug, &[&manifest.crate_name])?;
        }
        (true, false) => {
            lib_compile(&mut logger, &manifest, &lib_rs, &target_debug)?;
        }
        (false, true) => {
            bin_compile(&mut logger, &manifest, &main_rs, &target_debug, &vec![])?;
        }
...
pub fn build_tests() -> Result<()> {
    ...
    match (lib_rs.exists(), main_rs.exists()) {
        (true, true) => {
            test_compile(&mut logger, &manifest, &lib_rs, &target_tests, &vec![])?;
            lib_compile(&mut logger, &manifest, &lib_rs, &target_tests)?;
            test_compile(&mut logger, &manifest, &main_rs, &target_tests, &[&manifest.crate_name])?;
        }
        (true, false) => {
            test_compile(&mut logger, &manifest, &lib_rs, &target_tests, &vec![])?;
        }
        (false, true) => {
            test_compile(&mut logger, &manifest, &main_rs, &target_tests, &vec![])?;
        }
        (false, false) => return Err("There is nothing to compile".into()),
    }
    Ok(())
...
impl RustcBuilder {
    ...
    pub fn done(self) -> Rustc {
        Rustc {
            ...
            crate_type: self.crate_type.expect("crate type supplied"),
            crate_name: self.crate_name.expect("crate name supplied"),
            out_dir: self.out_dir.expect("out dir supplied"),
            lib_dir: self.lib_dir.expect("lib dir supplied"),
            cfg: self.cfg,
